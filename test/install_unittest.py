...
for s_line in fr:
    ...
    if (s_line.find('// Put physigym related parameter, variable, and vector action mapping here!') > -1):
        ...
        '\n',
        '                // update substrate concentration\n',
        '                set_microenv("substrate_a", parameters.doubles("subs_dose_a"));\n',
        '                set_microenv("substrate_b", parameters.doubles("subs_dose_b"));\n',
        '                set_microenv("substrate_c", parameters.doubles("subs_dose_c"));\n',
        '            }\n',
        '\n',
        ])
...
for s_line in fr:
    ...
    if (s_line.find("# model dependent action_space processing logic goes here!") > -1):
        ...
        "            'subs_dose_a': spaces.Box(low=0.0, high=1.0, shape=(1,), dtype=np.float64),\n",
        "            'subs_dose_b': spaces.Box(low=0.0, high=1.0, shape=(1,), dtype=np.float64),\n",
        "            'subs_dose_c': spaces.Box(low=0.0, high=1.0, shape=(1,), dtype=np.float64),\n",
        "        })\n",
        "\n",
        "        # output\n",
        ])
	...
	"        )\n",
        "\n",
        "        # substrate b\n",
	"        df_subs_b = pd.DataFrame(physicell.get_microenv('substrate_b'), columns=['x','y','z','substarte_b'])\n",
	...
	"        )\n",
        "\n",
        "        # substrate c\n",
	"        df_subs_c = pd.DataFrame(physicell.get_microenv('substrate_c'), columns=['x','y','z','substarte_c'])\n",
