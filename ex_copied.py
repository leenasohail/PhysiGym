...
class DecimalToBinaryLayer(nn.Module):
    def __init__(self, max_bits=32):
        super(DecimalToBinaryLayer, self).__init__()
        self.max_bits = max_bits

    def forward(self, x):
        # Get the shape of the input tensor
        shape = x.shape
        num_elements = x.numel()  # Total number of elements in the tensor
        # Flatten the tensor to 1D for easier processing
        x_flat = x.view(num_elements)

        # Initialize a tensor to store the binary representation
        binary_representation = torch.zeros(
            (num_elements, self.max_bits), device=x.device, dtype=torch.float32
        )

        # Perform the conversion for each element in the flattened tensor
        for i in range(self.max_bits):
            binary_representation[:, i] = x_flat % 2
            x_flat = x_flat // 2

        # Reverse the bits to get the correct order
        binary_representation = torch.flip(binary_representation, dims=[1])

        # Reshape back to the original shape with an additional dimension for bits
        new_shape = shape + (self.max_bits,)
        binary_representation = binary_representation.view(new_shape)
...
class QNetwork(nn.Module):
    def __init__(self, env):
        ...
        self.decimal_layer = DecimalToBinaryLayer(max_bits=9)

    def forward(self, x, a):
        state = self.decimal_layer(x).transpose(-2, -1).squeeze(-1)
        x = torch.cat([state, a], -1)
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
        x = self.fc3(x)
        return x
...
class Actor(nn.Module):
    ...
    def forward(self, x):
        x = self.decimal_layer(x).transpose(-2, -1).squeeze(-1)
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
