...
class Args:
    algo: str = "SAC"
    env: str = "physigym/ModelPhysiCellEnv-v0"
    tensorboard_log: str = ""
    trained_agent: str = ""
    ...
    log_interval: int = -1
    eval_freq: int = 25000
    optimization_log_path: Optional[str] = "./optimization_path/studies/"
    eval_episodes: int = 5
    n_eval_envs: int = 1
    save_freq: int = -1
    save_replay_buffer: bool = False
    ...
    study_name: Optional[str] = None
    verbose: int = 1
    gym_packages: List[str] = field(default_factory=list)
    env_kwargs: dict = field(default_factory=dict)
    eval_env_kwargs: dict = field(default_factory=dict)
    hyperparams: dict = field(default_factory=dict)
    conf_file: Optional[str] = None
    uuid: bool = False
    track: bool = False
    wandb_project_name: str = "SAC_IMAGE_SIMPLE_PHYSIGYM"
    wandb_entity: Optional[str] = "corporate-manu-sureli"
    progress: bool = False
    wandb_tags: List[str] = field(default_factory=list)


def train() -> None:
    args = tyro.cli(Args)  # Parse CLI arguments
    ...
    if env_id not in registered_envs:
        closest_match = difflib.get_close_matches(env_id, registered_envs, n=1)
        raise ValueError(f"{env_id} not found in gym registry, you may mean {closest_match[0]}?" if closest_match else "No close match found.")
    ...
    if args.track:
        try:
            ...
            raise ImportError("Install Weights & Biases with `pip install wandb`") from e
