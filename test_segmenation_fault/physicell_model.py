...
class ModelPhysiCellEnv(CorePhysiCellEnv):
    ...
    def get_observation_space(self):
        ...
        o_observation_space = spaces.Box(
            low=0, high=(2**16 - 1), shape=(1,), dtype=np.uint16
        )
    ...
    def get_observation(self):
        ...
        o_observation = np.array([i_cellcount], dtype=np.uint16)
    ...
    def get_img(self):
        ...
        df_conc = pd.DataFrame(
            physicell.get_microenv("drug"), columns=["x", "y", "z", "drug"]
        )
        ...
        df_variable = pd.DataFrame(
            physicell.get_variable("apoptosis_rate"), columns=["apoptosis_rate"]
        )
        df_cell = pd.merge(
            df_cell, df_variable, left_index=True, right_index=True, how="left"
        )
